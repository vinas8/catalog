# S1.2,11.1.01 - Real Snake Detail Completeness
...
## 📍 Scenario ID Breakdown

- **S1** → Shop Module
- **2.1** → Product Detail Flow
- **11.1** → KV Storage Integration
- **.1** → First scenario in this category
...
- ✅ ONE shared component (`SnakeDetailModal`) for Shop AND Farm
- ✅ Same base layout, same data fields
- ✅ Context-specific additions allowed (e.g., care stats in Farm)
...
```
SnakeDetailModal
├── renderHeader() - Name, species, morph
├── renderBasicInfo() - Species, morph, sex, age
├── renderPhysicalStats() - Weight, length
├── renderGeneticsSection() - Visual, hets, possible hets
├── renderBreederSection() - Breeder name, location, ID, reputation
├── renderCareInfo() - (Farm only) Care stats
└── renderPurchaseInfo() - (Shop only) Price, buy button
```
...
❌ Separate Shop detail component and Farm detail component  
❌ Showing different genetics in Shop vs Farm  
❌ Placeholder text like "Genetics coming soon"  
❌ Omitting breeder info  
❌ Showing only visual traits without hets  
