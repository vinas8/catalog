...
- ✅ Modules must only import from facades (`../module/index.js`)
- ✅ SMRI syntax validation (`S{M}.{RRR}.{II}`)
- ✅ Coverage tracking (which modules tested)
- ✅ Dead facade method detection
...
**Architecture:**
```
/src/components/  → UI layer (Navigation, SplitScreenDemo, modals)
/src/modules/     → Logic layer (S0-S9: shop, game, auth, etc.)
```
...
- ✅ Navigation, SnakeDetailModal (in use)
- ⏳ SplitScreenDemo, TestRenderer (ready)
...
```
┌──────────────────┐
│   Components     │  Generic UI (Navigation, SplitScreen, modals)
│   (Presentation) │  NO SMRI numbers, tracked via scenarios
└────────┬─────────┘
         │ imports ✅
         ↓
┌──────────────────┐
│     Modules      │  Business Logic (S0-S9)
│  (Business Layer)│  Exported via index.js facades
└────────┬─────────┘
         │ imports
         ↓
┌──────────────────┐
│      Config      │  Data (scenarios, species, constants)
│   (Data Layer)   │  Pure configuration
└──────────────────┘
```
...
## 📂 Directory Snapshot (Session End)

```
/root/catalog/
├── .smri/
│   ├── INDEX.md (906 lines - system rules)
│   ├── SESSION-START.md (smart start protocol)
│   ├── docs/ (business, technical, scenarios)
│   └── logs/ (session history)
├── src/
│   ├── components/ (7 UI components)
│   ├── modules/ (11 business logic modules S0-S9)
│   ├── config/ (scenarios, species, constants)
│   └── utils/ (helpers)
├── scripts/
│   ├── check-consistency.cjs
│   ├── check-architecture.cjs
│   ├── find-dead-facades.cjs
│   ├── smri-list-*.cjs (scenarios, functions, components)
│   └── README.md
├── debug/
│   ├── index.html (Debug Hub - 20 scenarios, 10/20 complete)
│   ├── demo-split-screen.html (split-screen example)
│   └── tools/ (smri-runner, test tools)
├── worker/ (Cloudflare Worker API)
├── tests/ (14 test files, 100% passing)
└── package.json (v0.7.7, 20+ npm scripts)
```

**Key Directories:**
- `.smri/` - Session management, docs, logs
- `src/modules/` - S0-S9 business logic (11 modules)
- `src/components/` - UI layer (7 components)
- `scripts/` - Dev tools (consistency, architecture, SMRI)
- `debug/` - Test runner, demos, tools
- `worker/` - Backend API (needs abstraction)

**Check tree on next start:**
```bash
tree -L 2 -I 'node_modules' /root/catalog
```
