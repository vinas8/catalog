# Serpent Town v0.5.0 - Complete System Documentation
...
## 🎯 What is Serpent Town v0.5.0?

Serpent Town is a **snake breeding e-commerce game** with real Stripe payments and Tamagotchi-style care mechanics, built using **SMRI (Scenario–Module–Relation–Instance)** as the universal system design language.
...
✅ **Traceable:** Every feature has a code  
✅ **Testable:** Scenarios map directly to tests  
✅ **Documentable:** User stories generate from scenarios  
✅ **Debuggable:** Debug hub runs scenarios by code  
✅ **Scalable:** Add scenarios without breaking existing ones
...
```
User → GitHub Pages (catalog.html)
  ↓
Stripe Checkout
  ↓
...
```
Shop(1) → Stripe(12.1) → Webhook(12.2) → Worker(5) → KV(11.2) → Game(2)
```

**Game State Flow (S2.5.01):**
```
Game(2) → LocalStorage → Worker(5) → KV(11.2)
```
 ...
 * Code: S4.5.02
 * Module: Payment (4) → Worker (5)
 */
```

**In Debug Logs:**
```
[2025-12-22T20:00:00Z] ▶ Running scenario: S4.5.02
[2025-12-22T20:00:01Z] ✅ S4.5.02 PASSED (1234ms)
```
...
2. **Integration Tests** - `npm run test:slow` - Frontend ↔ Backend
...
2. Switch to "🎯 SMRI Scenarios" tab
...
**Remaining (39 scenarios):** 🚧 In Progress
...
**Breaking Changes:**
- ⚠️ Debug moved to `src/modules/debug/index.html`
- ⚠️ SMRI codes changed format (S121 → S1.2.01)
- ⚠️ External services added (11-13)
...
```
S1.2.01      Shop → Game, instance 1
S5.11.2-12.2.01  Worker → KV + Webhooks
S3.5.01      Auth → Worker (security)
...
**Version:** 0.5.0  
**Status:** ✅ Production Ready with SMRI Infrastructure  
