...
TOTAL_PRODUCTS=$(jq '.products | length' "$SOURCE_FILE")
...
for i in $(seq 0 $((TOTAL_PRODUCTS - 1))); do
  NAME=$(jq -r ".products[$i].name" "$SOURCE_FILE")
  MORPH=$(jq -r ".products[$i].morph" "$SOURCE_FILE")
  GENDER=$(jq -r ".products[$i].gender" "$SOURCE_FILE")
  YOB=$(jq -r ".products[$i].yob" "$SOURCE_FILE")
  SPECIES=$(jq -r ".products[$i].species" "$SOURCE_FILE")
